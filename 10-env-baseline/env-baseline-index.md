...
| `_evidence/` | 原始输出（**逐字证据，不改写**）：`10_env_full_check.txt`（全面检测原始输出）、`01_defender.txt` ~ `09_commands.txt`（取证过程原始记录）、`_watch_log.jsonl`（5 分钟文件数采样）、`reorg-verify_20260919.txt`（整理结果逐项验证报告）、`09b_project_sacl_20260919.txt`（**项目树 Delete 审计 SACL 配置证据**，2026-09-19 补盲区）、`audit_4663_20260919_194x.txt`（目录消失事件的 4663 审计查询输出） |
